...
async function isFormand(pool, teamId, userId) {
...
async function isMember(pool, teamId, userId) {
    const result = await pool.request()
        .input('team_id', teamId)
        .input('user_id', userId)
...
router.get('/types/team/:teamId', auth_1.authMiddleware, async (req, res) => {
...
router.post('/types', auth_1.authMiddleware, async (req, res) => {
...
router.put('/types/:id', auth_1.authMiddleware, async (req, res) => {
    const { id } = req.params;
...
router.delete('/types/:id', auth_1.authMiddleware, async (req, res) => {
    const { id } = req.params;
    const userId = req.userId;
    try {
        const pool = await (0, db_1.getPool)();
...
router.get('/team/:teamId', auth_1.authMiddleware, async (req, res) => {
    const { teamId } = req.params;
    ...
    try {
        ...
        const inputs = [{ name: 'team_id', value: teamId }];
        if (paid !== undefined) {
            q += ' AND f.paid = @paid';
...
// Get team totals
router.get('/team/:teamId/totals', auth_1.authMiddleware, async (req, res) => {
    const { teamId } = req.params;
    const userId = req.userId;
    try {
        const pool = await (0, db_1.getPool)();
...
router.get('/team/:teamId/reasons', auth_1.authMiddleware, async (req, res) => {
    const { teamId } = req.params;
    const userId = req.userId;
    try {
        const pool = await (0, db_1.getPool)();
